...
function blogPostcontent(filename) {
  ...
  codelist.forEach(function (code) {
        ...
        var language = Htmlparser2.DomUtils.hasAttrib(code, "class") ? Htmlparser2.DomUtils.getAttributeValue(code, "class") : "language-javascript";
        var isMultiline = (codeText.includes('\n'));
        if (!isMultiline) {
          return ;
        }
        var highlighted = Prism$Blog.highlight(deHTML(codeText), language);
        Htmlparser2.DomUtils.replaceElement(code, Caml_array.get(Htmlparser2.parseDOM("<code class='multiline " + (language + ("'>" + (highlighted + "</code>")))), 0));
